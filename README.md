...
### Video Demo: <URL HERE>
### Introduction
VSEPR-Predictor (Valence Shell Electron Pair Repulsion - Predictor) is a cmd-line tool where a molecule is taken as input, and various different data is given as output—mainly electron and molecular geometries, which then yields other info like hybridization—with stepwise explanations as well. Calculating electron and molecular geometries *could* be done using quantum mechanics; however, this project's aim is to create a viable tool for students or others who would like to learn how to use VSEPR theory tangibly.

### **Limitations**
**It is important to preface** that *diatomic molecules*, like $\text{N}_2$ or $\text{O}_2$, and *radical molecules*, like $\text{C}\text{H}_3$ or $\text{N}\text{O}_2$, are **not** fully supported for this predictor. When given a diatomic molecule, the program outputs that the shape is linear; however, it doesn't include other data. This is because diatomics are fairly simple and do not need VSEPR theory, and the educational tool becomes arbitrary. As for radical molecules, they are too volatile to predict. Radical molecules have 1 electron acting as a sort of lone pair around the central atom. This 1 electron can sometimes affect the geometries or sometimes not. To figure this out, **quantum mechanics** must be used, which defeats the purpose of this tool.
...
**Example**: A hypothetical molecule with 4 Terminal atoms and 2 lone pairs would have a formula of **$AX_4E_2$** A subscript for X of 4 signifying 4 terminal atoms; a subscript of 2 for E signifying 2 Lone pairs for the central atom.
...
As stated, an abstract Lewis structure is created, and then electron configurations and the data we need are figured out from there. How, specifically, is this figured out using VSEPR-Predictor? First of all, the code parses input, only taking in actual molecules (using the Python library *chemicals* for assistance), and creates a list of dictionaries which includes element names and their subscripts. An example for H2O is shown here:
```json
[
...
*(where `element` is $\text{H}_2\text{O}$ or a parsed version).*
 ...
 The code then creates a sort of dot or Lewis structure, just abstractly. The code follows a similar way of creating the structure to how you would create it in real life, however, just more difficult and verbose. It's worth noting that this code does not create the hypothetical double and triple bonds for the respective molecule. This is because, as you can imagine, **we only need the** calculations for the lone pairs and **electron groups** in general. The approach taken is to take account of the total valence electrons we have, and then to account for some and subtract from the total, the goal being to 'use up' all of our valence electrons.

 ##### **Simplified Order of Steps**
The following is the simplified order of steps for the `fetch_structure_data(elements_and_nums, ve_total, central_atom)` function
...
2. **If we still have 'leftover valence electrons'**, which is in a lot of cases, we can spread the valence electrons upon each terminal atom so they all reach 8 valence electrons. If there are no more valence electrons to account for, we are done—at least for our purposes (read section above^^^).
...
VSEPR Predictor is a project that very mathematically and algorithmically calculates electron groups and lone pairs, needing prerequisite information to figure that out, and also yields extra information like bond hybridization as well. The approach, abstractly, is similar to how you would use VSEPR Theory on paper. However, the logic must be written differently and 'translated' into code. This way, the project was perfect for a first project to practice my skills, as it became a sort of challenge. **Thanks a lot for reading!**
